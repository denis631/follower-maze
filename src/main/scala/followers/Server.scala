...
object Server {
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      implicit val seqNumOrdering: Ordering[Event] = Ordering.by(_.sequenceNr)
      ...
      element => {
        ...
        val list = buffer.takeWhile { elem: Event =>
          if (elem.sequenceNr == seqNum) {
            seqNum += 1
            true
          } else {
            false
        ...
        buffer = buffer.drop(list.length)
  ...
  private def
  unimplementedFlow[A, B, C]: Flow[A, B, C] =
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue { value =>
